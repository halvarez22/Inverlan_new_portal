...
/**
 * Algoritmo de Scoring Dinámico para el Agente Inverlan.
 * SRP: Únicamente calcula la probabilidad de cierre del prospecto (Lead Score).
 */
function calculateLeadScore(lead) {
    let score = 0;
    // Puntos por calidad de contacto y perfil
    if (lead.providedPhone)
        score += 30; // El teléfono es crítico para el cierre
    if (lead.hasBudget)
        score += 20;
    if (lead.hasApprovedCredit)
        score += 30; // Lead de alta probabilidad
...
//# sourceMappingURL=leadQualifier.js.map